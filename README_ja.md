...
* ガントチャート画面の右クリックメニュー（コンテキストメニュー）にピン留め操作が表示されてしまう問題を修正。
* チケット関連（issues）以外の画面で、不要なJavaScriptやCSS、JSONデータがロードされないようガード処理を追加（アセット読み込みの最適化）。

v0.2.3 (2026-05-29)

* 設定画面のカラー値を16進コードのみ許可するよう検証し、インラインCSSへ出力する前にサニタイズ（潜在的な格納型XSSを修正）。
* ピン留めアイコン色のデフォルト値を追加し、設定が空のときにCSS変数が壊れる問題を修正。
* ピン操作の対象を可視チケット（Issue.visible）に限定し、チケット単位の閲覧権限を尊重するよう変更。
* ピンの同時操作（連打など）時の競合を握りつぶし、まれなエラーを回避。
